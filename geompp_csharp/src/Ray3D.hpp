...
public:
    ...
    bool Overlaps(Ray3D^ other);
    bool Overlaps(LineSegment3D^ segment);

    // Overlap — returns the shared geometry, or nullptr on miss / single-point touch
    // Overlap(Ray3D): returns Ray3D^ or LineSegment3D^ depending on direction; use 'is' pattern
    Ray3D^          Overlap(Line3D^ line);
    System::Object^ Overlap(Ray3D^ other);
