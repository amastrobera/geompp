...
std::vector<Point3D> convex_hull(std::vector<Point3D> const& points, std::optional<Vector3D> normal) {
  ...
  auto cv_indices =
      normal.has_value() ? detail::convex_hull_indices(points, normal.value()) : detail::convex_hull_indices(points);
...
Point3D lerp(Point3D const& P0, Point3D const& P1, double t) {
  // P(t) = (1 - t) * P0 + t * P1, rewritten without summing two points (operator+(Point3D, Point3D) is deleted)
  return P0 + t * (P1 - P0);
}
