...
bool GeometryCollection2D::AlmostEquals(GeometryCollection2D const& other, double epsilon) const {
...
GeometryCollection2D& GeometryCollection2D::operator=(GeometryCollection2D const& other) {
  if (this != &other) {
...
bool operator==(GeometryCollection2D const& lhs, GeometryCollection2D const& rhs) { return lhs.AlmostEquals(rhs); }
...
std::string GeometryCollection2D::ToWkt() const {
...
GeometryCollection2D GeometryCollection2D::FromWkt(std::string const& wkt) {
  try {
    GeometryCollection2D geoms;
...
void GeometryCollection2D::ToFile(std::string const& path) const {
  try {
...
GeometryCollection2D GeometryCollection2D::FromFile(std::string const& path) {
  try {
    std::string content;
