...
Polyline3D Polyline3D::Make(std::vector<Point3D> const& points) {
  ...
  return Polyline3D(std::move(unique_points));
}
...
std::vector<LineSegment3D> Polyline3D::ToSegments() const {
  std::vector<LineSegment3D> segs;

  for (int i = 0; i < KNOTS.size() - 1; ++i) {
    segs.push_back(LineSegment3D::Make(KNOTS[i], KNOTS[i + 1]));
  }

  return segs;
}

double Polyline3D::Length() const {
  auto iterable_range = ToSegments() | std::ranges::views::transform([](LineSegment3D const& s) { return s.Length(); });
