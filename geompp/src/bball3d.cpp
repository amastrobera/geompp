...
BBall3D::BBall3D(Points const& points) {
  ...
  double z_min = b[0].z(), z_max = b[0].z();
  std::size_t i_x_min = 0, i_x_max = 0, i_y_min = 0, i_y_max = 0, i_z_min = 0, i_z_max = 0;
  ...
  for (std::size_t i = 1; i < n; ++i) {
    ...
    if (compare(b[i].z(), z_min) < 0) {
      z_min = b[i].z();
      i_z_min = i;
    }
    if (compare(b[i].z(), z_max) > 0) {
      z_max = b[i].z();
      i_z_max = i;
    }
  }
  ...
  auto dz = b[i_z_max] - b[i_z_min];
  double dx2 = dx.Dot(dx);
  double dy2 = dy.Dot(dy);
  double dz2 = dz.Dot(dz);
  if (compare(dx2, dy2) >= 0) {
    if (compare(dx2, dz2) >= 0) {
      ...
      c = b[i_z_min] + dz / 2.0;
      rad2 = (b[i_z_max] - c).Dot(b[i_z_max] - c);
    }
  } else {
...
template BBall3D::BBall3D(std::vector<Point3D> const&);
