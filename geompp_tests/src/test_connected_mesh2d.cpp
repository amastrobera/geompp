...
class ConnectedMesh2DTest : public ::testing::Test {
 protected:
  void SetUp() override { g::DECIMAL_PRECISION = g::DP_THREE; }
  void TearDown() override { g::DECIMAL_PRECISION = g::DP_THREE; }
};

TEST_F(ConnectedMesh2DTest, FromTriangles_Empty_Throws) {
...
TEST_F(ConnectedMesh2DTest, OperatorBracket_OutOfRange_Throws) {
  auto t = g::Triangle2D::Make(g::Point2D(0, 0), g::Point2D(1, 0), g::Point2D(0, 1));
  auto mesh = g::ConnectedMesh2D::FromTriangles({t});
  EXPECT_THROW(mesh[1], std::out_of_range);
}

TEST_F(ConnectedMesh2DTest, OperatorBracket_SingleTriangle_MatchesInput) {
...
TEST_F(ConnectedMesh2DTest, FromTriangles_SharedEdge_WeldsToFourVertices) {
...
// Regression coverage for a fix in GridCellMapForConnectedMesh2D::Make() where the flat
...
// exercises exactly that path; see GridCellMapForConnectedMesh2DTest in test_grid_cell2d.cpp
// for direct assertions on the computed adjacency values.
TEST_F(ConnectedMesh2DTest, FromTriangles_ManyTriangles_LastVertexInBoundsAndCorrect) {
  ...
  auto mesh = g::ConnectedMesh2D::FromTriangles(triangles);
...
TEST_F(ConnectedMesh2DTest, Faces_MatchesOperatorBracket) {
  auto t0 = g::Triangle2D::Make(g::Point2D(0, 0), g::Point2D(1, 0), g::Point2D(1, 1));
  auto t1 = g::Triangle2D::Make(g::Point2D(0, 0), g::Point2D(1, 1), g::Point2D(0, 1));
  auto mesh = g::ConnectedMesh2D::FromTriangles({t0, t1});
...
// to triangle i+1's FIRST edge — see the matching GridCellMapForConnectedMesh2DTest.Make_Fan_* case
// in test_grid_cell2d.cpp for the raw adjacency assertions this walk relies on):
//
//   face 0: (0,0)-(1,0)-(1,1)       face 1: (0,0)-(1,1)-(0,1)
//   face 2: (0,0)-(0,1)-(-1,1)      face 3: (0,0)-(-1,1)-(-1,0)
//
...
TEST_F(ConnectedMesh2DTest, FaceView_Neighbor_WalksKnownAdjacencyPatternToExpectedTarget) {
...
TEST_F(ConnectedMesh2DTest, FaceView_Neighbor_SingleTriangle_AllEdgesAreBoundary) {
  using Edge = g::detail::TriangleCompactNeighborRef::TriangleEdge;
...
TEST_F(ConnectedMesh2DTest, FaceView_Neighbor_CrossingBackViaEntryEdge_ReturnsToOrigin) {
  using Edge = g::detail::TriangleCompactNeighborRef::TriangleEdge;
