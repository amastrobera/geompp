...
TEST_F(PlaneTest, Constructor) {
  ...
  auto xy_3pts = g::Plane::From3Points(g::Point3D(0, 0, 0), g::Point3D(1, 0, 0), g::Point3D(0, 1, 0));
  ASSERT_EQ(g::Point3D(0, 0, 0), xy_3pts.origin());
  ASSERT_TRUE(xy_3pts.normal().AlmostEquals(g::Vector3D(0, 0, 1)));

  // FromOriginAndNormal
  auto xy_norm = g::Plane::FromOriginAndNormal(g::Point3D(0, 0, 0), g::Vector3D(0, 0, 1));
  ASSERT_EQ(g::Point3D(0, 0, 0), xy_norm.origin());
  ASSERT_TRUE(xy_norm.normal().AlmostEquals(g::Vector3D(0, 0, 1)));

  // FromOriginAndAxes: explicit u=(1,0,0), v=(0,1,0) → normal = u×v = (0,0,1)
  auto xy_axes = g::Plane::FromOriginAndAxes(g::Point3D(0, 0, 0), g::Vector3D(1, 0, 0), g::Vector3D(0, 1, 0));
  ASSERT_EQ(g::Point3D(0, 0, 0), xy_axes.origin());
  ASSERT_TRUE(xy_axes.normal().AlmostEquals(g::Vector3D(0, 0, 1)));
  ASSERT_TRUE(xy_axes.axis_u().AlmostEquals(g::Vector3D(1, 0, 0)));
  ASSERT_TRUE(xy_axes.axis_v().AlmostEquals(g::Vector3D(0, 1, 0)));

  // Static convenience planes
  ASSERT_EQ(g::Point3D(0, 0, 0), g::Plane::XY().origin());
  ASSERT_TRUE(g::Plane::XY().normal().AlmostEquals(g::Vector3D(0, 0, 1)));
  ASSERT_TRUE(g::Plane::YZ().normal().AlmostEquals(g::Vector3D(1, 0, 0)));
  ASSERT_TRUE(g::Plane::ZX().normal().AlmostEquals(g::Vector3D(0, 1, 0)));

  // zero-length inputs throw
  EXPECT_ANY_THROW(g::Plane::FromOriginAndNormal(g::Point3D(0, 0, 0), g::Vector3D(0, 0, 0)));
  EXPECT_ANY_THROW(g::Plane::FromOriginAndAxes(g::Point3D(0, 0, 0), g::Vector3D(0, 0, 0), g::Vector3D(0, 1, 0)));
  EXPECT_ANY_THROW(g::Plane::FromOriginAndAxes(g::Point3D(0, 0, 0), g::Vector3D(1, 0, 0), g::Vector3D(0, 0, 0)));
}
...
TEST_F(PlaneTest, Assignment) {
  ...
  auto yz = g::Plane::YZ();
  yz = xy;
  ASSERT_EQ(g::Point3D(0, 0, 0), yz.origin());
  ASSERT_TRUE(yz.normal().AlmostEquals(g::Vector3D(0, 0, 1)));
}
...
TEST_F(PlaneTest, ProjectOnto) {
  ...
  EXPECT_EQ(g::Point3D(0, 0, 0), xy.ProjectOnto(g::Point3D(0, 0, -5)));
...
TEST_F(PlaneTest, ProjectInto) {
  ...
  auto xy = g::Plane::FromOriginAndAxes(g::Point3D(0, 0, 0), g::Vector3D(1, 0, 0), g::Vector3D(0, 1, 0));

  EXPECT_EQ(g::Point2D(5, 3), xy.ProjectInto(g::Point3D(5, 3, 7)));
  EXPECT_EQ(g::Point2D(0, 0), xy.ProjectInto(g::Point3D(0, 0, -5)));
...
TEST_F(PlaneTest, Evaluate) {
  ...
  EXPECT_EQ(g::Point3D(0, 0, 0), xy.Evaluate(g::Point2D(0, 0)));
...
TEST_F(PlaneTest, Contains) {
  ...
  ASSERT_TRUE(xy.Contains(g::Point3D(0, 0, 0)));
...
TEST_F(PlaneTest, IntersectionWLine) {
  ...
  auto x_line = g::Line3D::Make(g::Point3D(0, 0, 0), g::Point3D(1, 0, 0));
