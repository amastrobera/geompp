...
- `GeomPP.Transformations` namespace (`Transform.hpp`/`.cpp`, `TransformBuilder.hpp`/`.cpp`) — a static `Transform` class (`Translate`/`Rotate`/`Scale`/`Shear`/`Reflect` for the fast path, `Apply` for the general `transform(obj, matrix)` path — named `Apply` rather than `Transform` to avoid a method sharing its containing class's name) and a `TransformBuilder` sealed ref class (`Translate`/`Rotate`/`Scale`/`Shear`/`Reflect`/`Combine`, each returning `this` for chaining; `Get`/`Build`).
