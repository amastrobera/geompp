...
- `test_polygon2d.cpp`: `DISABLED_Randomized_*` (inclusion-exclusion, difference-partitions-subject, xor-as-symmetric-difference, results-are-simple) and `DISABLED_BooleanOp_MissedCrossing` — property-based guards that discovered the missed-crossing sweep bug above and now document it. Kept `DISABLED_` (they fail against the current engine); run with `--gtest_also_run_disabled_tests`.
