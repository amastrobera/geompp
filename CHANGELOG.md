...
**C++ (`geompp_tests`)**
- `test_calc_utils2d.cpp`: direct unit coverage for `SweepLine2D::ReverseRun` (previously untested directly, only exercised indirectly through the `has_intersections`/`find_intersections` algorithm-level tests and `Polygon2D`'s boolean-op tests) — `ReverseRun_ThrowsWhenFewerThanTwoIds`, `ReverseRun_StaleAnchor_ReturnsNotOk`, `ReverseRun_NonContiguousIds_ReturnsNotOk`, `ReverseRun_ThreeElementRun_ReversesWholeRunAndReportsOuterNeighbours`, `ReverseRun_SegIdsOrderDoesNotMatter_OnlyMembership`, and `ReverseRun_ProducesCallerOrderNotIdOrder` — the last one deliberately picks a crossing where id-ascending order and the geometrically-correct post-crossing order differ, so a future change that let `SweepLineComparator`'s `id1 < id2` tie-break (`sweep_line2d.cpp:201`) decide the reinsertion order instead of repositioning by index would fail it. Also new direct `SweepLineComparator` tests (`SweepLineComparator_OrdersByYAtCurrentSweepX`, `SweepLineComparator_TieBreaksByIdWhenGenuinelyCoincident`, `SweepLineComparator_ThrowsOnOutOfRangeId`) and an `EventQueue2D::Contains()` case for the reversed-`INTERSECTION`-pair match path (`EventQueue_Contains_MatchesReversedIntersectionPair`).
