...
- **Polygon boolean ops (2D and 3D) can drop a whole result component on rare configurations.** Randomized property tests (`DISABLED_Randomized_*` in `test_polygon2d.cpp`) found that roughly 1 in a few hundred random convex operand pairs — even well-conditioned ones — grossly violates identities like `area(A) == area(A∩B) + area(A∖B)`. Root cause, traced to a minimal deterministic repro (`DISABLED_BooleanOp_MissedCrossing`): the shared Bentley-Ottmann sweep behind `find_intersections()` occasionally **misses a genuine segment crossing** (verified analytically for the repro — two input segments that provably intersect at in-range parameters are not reported). The split step then leaves a fragment straddling the other operand's boundary, and the component beyond the missed crossing is lost. This is upstream of the boolean-op classification (it corrupts the arrangement itself) and reproduces regardless of how edges are classified — confirmed unchanged by the `boolean_op_multi` classification rewrite below, which fixed a different, unrelated issue. It is documented by the three still-`DISABLED_` tests (`DifferencePartitionsSubject`, `ResultsAreSimple`, `BooleanOp_MissedCrossing`; run with `--gtest_also_run_disabled_tests`) and tracked as future work — fixing the sweep's missed-crossing case is a core-algorithm change out of scope here.
