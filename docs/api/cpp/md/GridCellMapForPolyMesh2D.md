...
**static** `GridCellMapForPolyMesh2D Make(std::vector< `[`Polygon2D`](Polygon2D.md)` > const & polygons)`


**Parameters**

- `polygons` (std::vector< [`Polygon2D`](Polygon2D.md) > const &)
