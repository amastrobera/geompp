...
**static** `GridCellMapForPolyMesh3D Make(std::vector< `[`Polygon3D`](Polygon3D.md)` > const & polygons)`


**Parameters**

- `polygons` (std::vector< [`Polygon3D`](Polygon3D.md) > const &)

## `GetUniques`

`std::shared_ptr< std::vector< `[`Point3D`](Point3D.md)` > > GetUniques() const`
