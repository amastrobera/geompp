...
**static** `Mesh3D FromTriangles(std::vector< `[`Triangle3D`](Triangle3D.md)` > const & triangles, AdjacencyConformity conformity)`
...
[`ConnectedMesh3D`](ConnectedMesh3D.md)` Connect() const`
