...
**static** `GridCell2D FromPoint(`[`Point2D`](Point2D.md)` const & p, double epsilon)`
...
- `epsilon` (`double`) — The cell side length. Points within epsilon of each other are not guaranteed to land in the same cell if they straddle a cell boundary; points up to sqrt(2) * epsilon apart can land in the same cell if they don't.
