...
- `seg_id` (`std::size_t`) — index of the SegmentList [0, N-1]
...
- `seg_ids` (`std::vector< std::size_t > const &`) — the ids expected to form one contiguous run, in any order; must have size() >= 2.
