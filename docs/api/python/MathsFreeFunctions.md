# `MathsFreeFunctions` (Python)

Free functions in `geompp::maths` (not methods on a class — call them directly).

## `solve_gauss`

`solve_gauss(a: `[`Matrix`](Matrix.md)` < T, N, N >, b: `[`Vector`](Vector.md)` < T, N >) -> `[`Vector`](Vector.md)` < T, N >`
...
`solve_cramer(a: `[`Matrix`](Matrix.md)` < T, N, N >, b: `[`Vector`](Vector.md)` < T, N >) -> `[`Vector`](Vector.md)` < T, N >`
