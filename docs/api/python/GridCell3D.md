...
**static** `from_point(p: `[`Point3D`](Point3D.md)`, epsilon: float) -> GridCell3D`
...
- `epsilon` (`float`) — The cell side length. Points within epsilon of each other are not guaranteed to land in the same cell if they straddle a cell boundary; points up to sqrt(3) * epsilon apart can land in the same cell if they don't.
