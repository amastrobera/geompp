...
**static** `Mesh2D^ FromTriangles(std::vector< `[`Triangle2D`](Triangle2D.md)` > triangles, AdjacencyConformity^ conformity)`
...
- `triangles` (std::vector< [`Triangle2D`](Triangle2D.md) >) — The triangles to weld into a mesh. Order is not required to reflect adjacency.
...
**static** `Mesh2D^ FromWkt(System::String^ wkt)`
...
**static** `Mesh2D^ FromFile(System::String^ path)`
...
[`GeometryCollection2D`](GeometryCollection2D.md)`^ ToGeometryCollection()`
