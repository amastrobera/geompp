...
[`Triangle3D`](Triangle3D.md)`^ Geometry()`

facet geometry, rebuilt as a [Triangle3D](Triangle3D.md) from the mesh's welded vertices.

## `Neighbor`

`FaceView3D^  (nullable) Neighbor(detail::TriangleCompactNeighborRef::TriangleEdge edge_id)`
...
`detail::TriangleCompactNeighborRef::TriangleEdge NeighborEntryEdge(detail::TriangleCompactNeighborRef::TriangleEdge edge_id)`
